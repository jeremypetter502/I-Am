<script>
  ...
  function setResponse(value) {
    ...
    const nextCurrent = firstUnanswered === -1
      ...
      : firstUnanswered;
    try {
      sessionService.saveProgress('aesthetics', { responses: nextResponses, current: nextCurrent, expectedLength: questions.length });
      const progressDetail = { module: 'aesthetics', responses: nextResponses.slice(0), current: nextCurrent, expectedLength: questions.length };
      dispatch('moduleprogress', progressDetail);
      if (typeof onProgress === 'function') onProgress(progressDetail);
      if (!wasComplete && nowComplete) {
        const result = scoreAestheticsIfAvailable(nextResponses) || {
        ...
        sessionService.saveProgress('aesthetics', { responses: nextResponses.slice(0), current: nextResponses.length, expectedLength: questions.length, completed: true });
        dispatch('complete', { module: 'aesthetics', responses: nextResponses.slice(0), result });
      }
    } catch (e) {
...
{#if loading}
  <div class="state-card">
    ...
    <h3>Fetching the aesthetics questions…</h3>
    <p>Getting the visual taste module ready.</p>
  </div>
{:else if error}
