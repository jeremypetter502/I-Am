<script>
  ...
  function setResponse(value) { responses[current] = value; }
  function handleAnswer(value){ setResponse(value); setTimeout(()=>{ if (current < questions.length-1) current += 1; }, 120); }
  ...
  async function submit(){
    ...
    const result = scoreMusicIfAvailable(responses) || { raw: { total: responses.reduce((a,b)=>a+(Number(b)||0),0) }, normalized: { total: Math.round((responses.reduce((a,b)=>a+(Number(b)||0),0)/(responses.length*5||1))*10000)/100 } };
    dispatch('complete', { module: 'music', responses: responses.slice(0), result });
  }
</script>

{#if loading}
  <div>Loading music questions…</div>
{:else if error}
