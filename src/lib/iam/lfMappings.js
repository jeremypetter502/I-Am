...
export class LfMappings {
  constructor(specsDir) {
    // `path.resolve` is Node-only and externalized in browser bundles, so avoid calling
    // defaultSpecsDir() there entirely rather than only skipping the fs reads in load().
    this.specsDir = specsDir || (typeof window === 'undefined' ? defaultSpecsDir() : null);
    this.segmentMap = {};
